...
describe("withdraw-liquidity", () => {
  try {
    ...
    const depositor = loadKeypairFromFile("/Users/mac/.config/solana/id.json");
    ...
    before(async () => {
      ...
      const amountB = new BN(20 * 10 ** mintBDecimals);
      ...
      console.log("Amount LP Deposit", depisitorLP.amount.toString());
    });

    it("withdraw-liquidity", async () => {
      const liquidity = new BN(14);
      const tx = await program.methods
      ...
      assert(
        poolATokenAccount.amount.toString() == "10",
        "Correct amount token A"
      );

      assert(
        poolBTokenAccount.amount.toString() == "20",
        "Correct amount token B"
      );
    });
  } catch (error) {
    console.error(error);
